...
import { useBookingState, type Location } from "@/hooks/use-booking-state"
  // adjust to "../components/success" if you don't use @ paths
...
export function TransportOptions({
  ...
  // Fallback Haversine if routeDistanceKm not yet available
  const haversineKm = useMemo(() => {
  ...
  const distanceKm = routeDistanceKm ?? haversineKm
  const baseDurationMin =
    routeDurationMin ?? Math.max(5, Math.round(distanceKm * 2)) // ~30 km/h fallback

  const etaFor = (multiplier: number) => `${Math.max(3, Math.round(baseDurationMin * multiplier))} mins`
  const fareFor = (ratePerKm: number) => Math.max(30, Math.round(distanceKm * ratePerKm))
...
const handleBooking = (option: TransportOption) => {
  ...
  toast({
    ...
    description: `Your ${option.name} has been booked. Redirecting...`,
  })

  const params = new URLSearchParams({
    amount: option.fare.toString(),
    carType: option.type,
    transportName: option.name,
    sourceLocation: sourceLocation.name,
    destination: destination.name,
    distance: (routeDistanceKm ?? 0).toFixed(1),
    paymentStatus: "paid",
    ...
    >
      <Card className="bg-white/95 backdrop-blur-sm shadow-xl border-green-100">
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Choose your preferred transport for the journey ({distanceKm.toFixed(1)} km)
          </CardDescription>
        </CardHeader>
